            ...
            public IStatus run(IProgressMonitor monitor)
            {
                ...
                Display.getDefault().syncExec(new Runnable()
                {
                    ...
                    public void run()
                    {
                        ...
                        typeTreeViewer.add(QuickTypeHierarchyView.this.rootNode, new TempLoadingNode());
                    }
                ...
                if (showAnc)
                {
                    ...
                    try
                    {
                        ...
                        if(topIndex == 0)
                        {
                            typeRoot = new TypeInfo(rootType,TypeInfo.OBJECT_TYPE,docbase);
                            //System.out.println("created TypeInfo since indx = 0");
                        }
                        else
                        {
                            typeRoot = new QuickTypeInfo(rootType,TypeInfo.OBJECT_TYPE,docbase);
    ...
    protected void createToolbar()
    {
        setType = new Action("Set Type")
        {
            public void run()
            {
                ...
                InputDialog id = new InputDialog(sh,"Type Name","Show Type Hierarchy For","",null);
                int status = id.open();
                if(status == InputDialog.OK)
                {
                    ...
                    if(val != null && val.length() > 0)
                    {
                        ...
                        QuickTypeHierarchyView.this.showHierarchy(val, docbase, true);
                    }
                }
            }
        };
